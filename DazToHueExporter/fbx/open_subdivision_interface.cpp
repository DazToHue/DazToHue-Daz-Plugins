...
FbxMesh* SubdivideFbxScene::SubdivideMesh(FbxNode* pNode, FbxMesh* pMesh, int subdLevel)
{
	...
	// Get cluster and skin weight data from FBX geometry links and interpolate them
	...
	for (int i = 0; i != lSkinCount; ++i)
	{
		...
		for (int j = 0; j != lClusterCount; ++j)
		{
			...
			//QMessageBox::information(0, "DazToHue Exporter", QString::fromUtf8(lCluster->GetNameWithNameSpacePrefix().Buffer()));
